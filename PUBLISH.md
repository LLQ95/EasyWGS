...
本地仓库框架已经就绪，GitHub 仓库将创建在已授权账号 LLQ95 名下，名称 EasyWGS。
...
git init -b main
git add .
git commit -m "init EasyWGS v0.1.0: short/long/hybrid WGS pipeline + bilingual guidebook"
```

## 二、创建 GitHub 仓库并推送

方式 A（网页）：在 https://github.com/new 建名为 EasyWGS 的仓库，不要自动生成
README/.gitignore/LICENSE（本地已具备），然后：

```bash
git remote add origin https://github.com/LLQ95/EasyWGS.git
...
```bash
gh repo create EasyWGS --public --source=. --remote=origin --push
```
